...
if [ $CUR_BRANCH == "master" ] ; then
    if [ "$TESTED" != "" ] ; then
        ...
        git stash pop
        if [ "$COMPILED" != "" ] ; then
            ( git commit -aqm "Build $BUILD_ID" && git push -q --set-upstream origin "$DAILY_BRANCH" > /dev/null ) &
        else
            ( git commit -aqm "Temp $BUILD_ID"  && git push -q --set-upstream origin "$DAILY_BRANCH" > /dev/null ) &
        fi
    fi
