...
fromIndices [d] s f = mergeScalars $ 
    FiniteTensor (Covariant s [d]) $ Boxed.generate s (Scalar . f)
...
const [d] s v = mergeScalars $ 
    FiniteTensor (Covariant s [d]) $ Boxed.replicate (fromIntegral s) (Scalar v)
...
randomDouble [i] s d = do
  ...
  return $ mergeScalars $ FiniteTensor (Covariant s [i]) $ Scalar <$> Boxed.fromList components
...
fromCSV [i] fileName separator = do
  ...
  if readSize > 0
  then return $ mergeScalars $ FiniteTensor (Covariant readSize [i]) (Scalar <$> Boxed.fromList (rights components))
