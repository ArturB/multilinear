...
randomDouble [i] s d = do
  ...
  return $ mergeScalars $ FiniteTensor (Covariant s [i]) $ ZipVector $ Scalar <$> Boxed.fromList components
...
fromCSV [i] fileName separator = do
  ...
  if size > 0
  then return $ mergeScalars $ FiniteTensor (Covariant size [i]) $ ZipVector (Scalar <$> Boxed.fromList (rights components))
